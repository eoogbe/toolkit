...
public class Adviser {
  ...
  private String getSuggestion(ConfigNode node) {
    ConfigGenerator snippetGenerator = new ConfigGenerator(4, Comment.Type.NONE);
    snippetGenerator.visit(node.getChild());
    return String.format("%nDid you mean:%n  %s:%n%s", node.getText(), snippetGenerator.toString());
  }
}
