  ...
  public ConfigNode mergeLanguageSettings(
    ...
    if (!NodeFinder.hasChild(languageSettingsNode)) {
          ...
          ListTransformer.generateList(
              ...
              new ListTransformer.ElementTransformer<Map.Entry<String, LanguageFormatter>>() {
                ...
                public ConfigNode generateElement(
                    int startLine, Map.Entry<String, LanguageFormatter> entry) {
      ...
      return languageSettingsNode
          ...
          .setComment(
              new RejectRefreshComment("The settings of generated code in a specific language."))
          .setAdvice(
              Advice.newBuilder()
                  .rule(Rule.LANGUAGE_SETTINGS)
                  .comment("Missing 'language_settings'")
                  .location(helper.getLocation(languageSettingsNode.getStartLine()))
                  .hasSuggestion(true)
                  .build());
    }
    ...
    for (Map.Entry<String, LanguageFormatter> entry : LANGUAGE_FORMATTERS.entrySet()) {
      ...
      if (NodeFinder.hasChild(languageNode)) {
        ...
        if (!NodeFinder.hasChild(packageNameNode)) {
          packageNameNode.setAdvice(
              Advice.newBuilder()
                  ...
                  .location(helper.getLocation(packageNameNode.getStartLine()))
                  .hasSuggestion(true)
                  .build());
        ...
        languageNode
            .setComment(
                new RejectInitialComment(String.format("Check added language '%s'", language)))
            .setAdvice(
                Advice.newBuilder()
                    .rule(Rule.LANGUAGE_SETTINGS)
                    .comment(String.format("Missing language settings for key '%s'", language))
                    .location(helper.getLocation(languageNode.getStartLine()))
                    .hasSuggestion(true)
                    .build());
        mergeLanguageSetting(languageNode, languageFormatter, packageName);
      }
    }
